#!/usr/bin/env python3
"""
Interactive Point Cloud Explorer for Collaborative Perception

This module provides an advanced interactive visualization tool for exploring
collaborative perception datasets from warehouse robotics. It features temporal
navigation, multi-robot trajectory visualization, point cloud analysis, and
comprehensive interactive controls for detailed data exploration.

Key Features:
- Interactive temporal navigation with time slider controls
- Multi-robot trajectory visualization with trail tracking
- Real-time point cloud exploration and filtering
- 2D/3D visualization modes with seamless switching
- Warehouse infrastructure overlay and workstation visualization
- Animation controls for temporal data playback
- Comprehensive data analysis and statistics display

The explorer is essential for understanding collaborative perception data quality,
robot behavior patterns, and sensor coverage in warehouse environments.
...
class CollaborativePerceptionInteractiveExplorer:
    """
    Advanced Interactive Explorer for Collaborative Perception Data.

    This class provides comprehensive interactive visualization capabilities for
    exploring collaborative perception datasets from warehouse robotics. It enables
    temporal navigation, multi-robot analysis, and detailed point cloud exploration
    with intuitive controls and real-time feedback.

    Key Capabilities:
    - Temporal data navigation with smooth animation controls
    - Multi-robot trajectory visualization and comparison
    - Interactive point cloud filtering and analysis
    - Warehouse infrastructure and workstation overlay
    - Real-time statistics and data quality assessment
    - Export capabilities for presentations and documentation
    """

    def __init__(self, collaborative_data_path: Optional[str] = None):
        """
        Initialize the collaborative perception interactive explorer.

        Args:
            collaborative_data_path (str, optional): Path to transformed collaborative data CSV file
                                                    containing multi-robot sensor data
        """
        self.collaborative_data_path = collaborative_data_path
        self.loaded_collaborative_data = None

        # Animation and interaction state
        self.current_temporal_frame = 0
        self.animation_playing = False
        self.animation_speed_multiplier = 1.0
        self.display_point_clouds = True
        self.display_robot_trails = True
        self.visualization_mode = '2d'  # '2d' or '3d'

        # Define warehouse environment boundaries (adjustable for different facilities)
        self.warehouse_spatial_boundaries = {
            'x_coordinate_min': -9.1, 'x_coordinate_max': 10.2,
            'y_coordinate_min': -4.42, 'y_coordinate_max': 5.5,
            'z_coordinate_min': 0.0, 'z_coordinate_max': 3.0
        }

        # Define known workstation positions in the warehouse (ground truth)
        self.warehouse_workstation_positions = {
...
def main():
    ...
    if args.input:
        ...
        # Create collaborative perception explorer and run
        explorer = CollaborativePerceptionInteractiveExplorer(data_path)
        explorer.visualization_mode = args.plot_type

        if explorer.load_data():
            explorer.run_visualization()
        else:
